...
#  Description: Further recursion practice 
  ...
  if target == 0:
    ...
    if nums[start] == target:
      ...
      if start <= len(nums) - 3:
        ...
        if start == len(nums) - 2 and nums[start] == target:
          return True 
        else:
...
def splitArray(nums):
  idx = 0
  b = []
  if splitArrayHelper(nums, b, idx) == True:
    return True 
  else:
    return False 
  
def splitArrayHelper(nums, b, idx):
  if (idx == len(nums)):
    if (2 * sum(b) == sum(nums)):
      return True
    return 
    ...
    c = b[:]
    b.append(nums[idx])
    return splitArrayHelper(nums, b, idx + 1) or splitArrayHelper(nums, c, idx + 1)
...
def splitOdd10(nums):
  if splitOdd10Helper(nums, 0, 0, 0) == True:
    return True 
  else:
    return False 


def splitOdd10Helper(nums, start1, starttwo, startthree):
  if start1 == len(nums): 
    return (startthree % 2 == 1 and starttwo % 2 == 2) or (starttwo % 2 == 1 and startthree % 10 == 0)
  if splitOdd10Helper(nums, start1 + 1, starttwo + nums[start1], startthree): 
    return True 
  return splitOdd10Helper(nums, start1 + 1, starttwo, startthree + nums[start1])
...
def split53(nums):
  if split53Helper(0, nums, 0) == True:
    return True 
  else:
    return False 

def split53Helper(won, nums, diffyq):
  if won == len(nums): 
    return(diffyq == 0)
  elif nums[won] % 5 == 0: 
    return split53Helper(won + 1, nums, diffyq + nums[won])
  elif nums[won] % 3 == 0: 
    return split53Helper(won + 1, nums, diffyq - nums[won])
  elif split53Helper(won + 1, nums, diffyq + nums[won]): 
    return True 
  return split53Helper(won + 1, nums, diffyq - nums[won])
