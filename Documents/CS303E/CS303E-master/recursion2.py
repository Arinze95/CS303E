...
#  Description: Recursion problems part 2!
...
def parenBit(str):
  if ")" == str[0]: 
    ...
    if "(" == str[0]: 
      return str[0] + parenBit(str[1:])
    elif "(" not in str and ")" in str: 
      return str[0] + parenBit(str[1:])
    else: 
...
def strDist(str, sub):
  if len(sub) > len(str): 
    return 0
  if str[0:len(sub)] == sub and sub == str[-len(sub):]: 
    return len(str)
  elif sub != str[0:len(sub)] and sub == str[-len(sub):]: 
